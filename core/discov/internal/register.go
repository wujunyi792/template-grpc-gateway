...
import (
	...
	"fmt"
	"io"
	"pinnacle-primary-be/core/contextx"
...
type cluster struct {
	...
	watchGroup *threadx.RoutineGroup
...
func newCluster(endpoints []string) *cluster {
	return &cluster{
		...
		watchGroup: threadx.NewRoutineGroup(),
...
func (c *cluster) handleWatchEvents(key string, events []*clientv3.Event) {
	...
	for _, ev := range events {
		...
		default:
			print("Unknown event type: %v", ev.Type)
		}
	}
}
...
func (c *cluster) reload(cli EtcdClient) {
	...
	close(c.done)
	c.watchGroup.Wait()
	c.done = make(chan langx.PlaceholderType)
	c.watchGroup = threadx.NewRoutineGroup()
	var keys []string
...
func (c *cluster) watchStream(cli EtcdClient, key string, rev int64) bool {
	...
	for {
		...
		case wresp, ok := <-rch:
			if !ok {
				print("etcd monitor chan has been closed")
				return false
			}
			if wresp.Canceled {
				print("etcd monitor chan has been canceled, error: %v", wresp.Err())
				return false
			}
			if wresp.Err() != nil {
				print(fmt.Sprintf("etcd monitor chan error: %v", wresp.Err()))
				return false
			}
