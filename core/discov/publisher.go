...
import (
	...
	"os"
	"pinnacle-primary-be/core/discov/internal"
	"pinnacle-primary-be/core/langx"
	"pinnacle-primary-be/core/syncx"
	"pinnacle-primary-be/core/systemx"
	"pinnacle-primary-be/core/threadx"
	"time"
)
...
type (
	...
	Publisher struct {
		...
		quit       *syncx.DoneChan
		pauseChan  chan langx.PlaceholderType
		resumeChan chan langx.PlaceholderType
	}
)
...
func NewPublisher(endpoints []string, key, value string, opts ...PubOption) *Publisher {
	publisher := &Publisher{
		...
		pauseChan:  make(chan langx.PlaceholderType),
		resumeChan: make(chan langx.PlaceholderType),
	}
...
func (p *Publisher) doKeepAlive() error {
	...
	for range ticker.C {
		...
		default:
			cli, err := p.doRegister()
			if err != nil {
				log.Error("etcd publisher doRegister: ", err.Error())
				break
			}

			if err := p.keepAliveAsync(cli); err != nil {
				log.Error("etcd publisher keepAliveAsync: ", err.Error())
				break
			}
			...
			case _, ok := <-ch:
				if !ok {
					...
					if err := p.doKeepAlive(); err != nil {
						log.Error("etcd publisher KeepAlive: ", err.Error())
					}
					return
				}
			case <-p.pauseChan:
				log.Info(fmt.Sprintf("paused etcd renew, key: %s, value: %s", p.key, p.value))
				p.revoke(cli)
				select {
				case <-p.resumeChan:
					if err := p.doKeepAlive(); err != nil {
...
func (p *Publisher) revoke(cli internal.EtcdClient) {
	if _, err := cli.Revoke(cli.Ctx(), p.lease); err != nil {
		log.Error("etcd publisher revoke: ", err.Error())
	}
}
